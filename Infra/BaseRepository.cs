...
namespace Abc.Infra
{
    public abstract class BaseRepository<TDomain, TData> : ICrudMethods<TDomain> where TData : PeriodData, new() where TDomain : Entity<TData>, new()
    {
        ...
        {
            var query = createSqlQuery(); //создается запрос
            var set = await runSqlQueryAsync(query); //спрашиваются данные
